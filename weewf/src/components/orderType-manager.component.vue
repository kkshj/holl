<template>
  <div>
    <el-row class="header">
      <el-col :span="18"><h4>订单类型管理</h4></el-col>
      <el-col :span="6">
    ...
    <el-row class="search">
      <el-col :span="20">
        ...
        <el-select
          ...
          filterable
          remote
          placeholder="订单类型名称（请输入关键字）"
          :remote-method="remoteOrderTypeNameMethod">
          <el-option
            v-for="item in orderTypeNameOptions"
            :key="item.text"
            :label="item.text"
            :value="item.text">
          </el-option>
    ...
    <el-table :data="orderTypelist"  v-loading="loading" style="width: 100%">
      ...
      <el-table-column align="left" prop="text" label="订单类型名称"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      goToDetail (type,id) {
        //console.log(id)
        this.$router.push({
          path: '/order-type-manager/detail',
          query: {
            type: type,
            id: id
          }
